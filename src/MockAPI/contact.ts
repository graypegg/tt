...
export const ContactHandlers = [
  ...
  rest.post<ContactEndpoint['POST contact']['body']>(
    '/contact',
    (req, res, ctx) => {
      if (isContact(req.body)) {
        const newContact = { ...req.body, id: (++STORE.lastId).toString() }
        STORE.contacts = STORE.contacts.concat([newContact])
        return res(ctx.status(200), ctx.json(newContact))
      }
      return res(ctx.status(400))
    }
  ),
  rest.put<ContactEndpoint['PUT contact']['body']>(
    '/contact/:id',
    (req, res, ctx) => {
  ...
  rest.delete<ContactEndpoint['PUT contact']['body']>(
    '/contact/:id',
    (req, res, ctx) => {
      if (req.params['id']) {
